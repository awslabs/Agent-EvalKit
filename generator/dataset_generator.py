...
from .data_types.evaluation import Interaction
from .prompt_templates.prompt_templates import generate_case_template as CASE_SYSTEM_PROMPT
...
class DatasetGenerator(Generic[InputT, OutputT]):
    """
    Generates evaluation datasets with test cases for agent assessment.

    This class creates structured test cases tailored to specific tasks
    ...
    async def from_context_async(
        ...
        Args:
            ...
            num_cases: Number of test cases to generate
            num_topics: Number of topics for diverse coverage (default: 5 if not specified)

        Returns:
            Dataset containing context-based test cases.
        """
        # Default to 5 topics if not specified
        if num_topics is None:
            num_topics = 5
        ...
        cases = await self.generate_cases_async(
            ...
            f"""answered using the provided context for this task: {task_description}""",
            num_cases=num_cases,
