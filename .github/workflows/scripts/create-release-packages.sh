...
# Build Agent-EvalKit template release archives for each supported AI assistant and script type.
...
generate_commands() {
  ...
  for command in commands/*.md; do
    ...
    case $ext in
      toml)
        body=$(printf '%s\n' "$body" | sed 's/\\/\\\\/g')
        { echo "description = \"$description\""; echo; echo "prompt = \"\"\""; echo "$body"; echo "\"\"\""; } > "$output_dir/evalkit.$name.$ext" ;;
      md)
        echo "$body" > "$output_dir/evalkit.$name.$ext" ;;
      prompt.md)
        echo "$body" > "$output_dir/evalkit.$name.$ext" ;;
    esac
  done
...
build_variant() {
  ...
  local base_dir="$GENRELEASES_DIR/evalkit-${agent}-package-${script}"
  echo "Building $agent ($script) package..."
  mkdir -p "$base_dir"
  
  # Copy base structure but filter scripts by variant
  EVALKIT_DIR="$base_dir/.evalkit"
  mkdir -p "$EVALKIT_DIR"
  
  [[ -d memory ]] && { cp -r memory "$EVALKIT_DIR/"; echo "Copied memory -> .evalkit"; }
  
  # Only copy the relevant script variant directory
  if [[ -d scripts ]]; then
    mkdir -p "$EVALKIT_DIR/scripts"
    case $script in
      sh)
        [[ -d scripts/bash ]] && { cp -r scripts/bash "$EVALKIT_DIR/scripts/"; echo "Copied scripts/bash -> .evalkit/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$EVALKIT_DIR/scripts/" \; 2>/dev/null || true
        ;;
      ps)
        [[ -d scripts/powershell ]] && { cp -r scripts/powershell "$EVALKIT_DIR/scripts/"; echo "Copied scripts/powershell -> .evalkit/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$EVALKIT_DIR/scripts/" \; 2>/dev/null || true
        ;;
    esac
...
ls -1 "$GENRELEASES_DIR"/evalkit-template-*-"${NEW_VERSION}".zip
